...
void MakeDistinctMirrorPairs (Node* &headPtr)
{
    ...
    while (head != 0)
    {
        while(curr != 0)
        {
            // three cases to handle duplicates
            if (head->data == curr->data && head->link == curr)
            {
                dup = 1;
                tail = head;
                curr->link = head->link;
                curr = head->link; //current points to current 
                continue;
            }
         
            else if (head->data == curr->data && dup == 0)
            {
                dup = 1;
                prev->link = curr->link; //shift current to previous
                tail = head;
                curr->link = head->link; // shifts head to current
            ...
            {
                prev->link = curr->link; // give prev link curr's link to head pointer
                delete curr;
                curr = prev->link; //curr's data is now linked to previous
                continue;
            }
        ...
        if (dup == 0)
        {
            ...
            dupNode->data = head->data;
            dupNode->link = head->link;
            head->link = dupNode;
        }
        head = head->link->link; //head is assigned the dupNode's link 
        dup = 0;
        //get the next element if there are more
        if (head != 0)
        {
            prev = head;
