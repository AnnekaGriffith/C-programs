...
IntSet IntSet::subtract(const IntSet& otherIntSet) const
{
    ...
    for (int i = 0; i < otherIntSet.used; ++i)
    {
        if ((dataLoad.contains(otherIntSet.data[i])) == true)
        {
            dataLoad.remove(otherIntSet.data[i]);
        }
    }
...
bool IntSet::add(int anInt)
{
    ...
    {
        if (size() >= capacity)
            this->resize((1.5 * this->capacity) + 1 );
        this->data[size()] = anInt;
        this->used++;
        resize(used);
        return true;
    }
